...
#if 0
// activate and manipulate to test MTU negotiation and fragmentation
#define MTU_TESTING
#define CONNECT_REQ_MTU             2000    // will expect 1691 after negotiation
#define ACCEPT_MTU                  1900
#define FIRST_BUFFER_TEST_SEND_SIZE 1700
#endif
...
const UINT WIDCOMM_WM_ON_CONNECTED_L2CAP = RegisterWindowMessage(_T("WIDCOMM_WM_ON_CONNECTED_L2CAP"));
const UINT WIDCOMM_WM_ON_REMOTE_DISCONNECTED_L2CAP = RegisterWindowMessage(_T("WIDCOMM_WM_ON_REMOTE_DISCONNECTED_L2CAP"));
...
BEGIN_MESSAGE_MAP(CBlueTimeDlg, CDialog)
    ...
    ON_REGISTERED_MESSAGE(WIDCOMM_WM_ON_CONNECTED_L2CAP, OnConnectedMessage)
    ON_REGISTERED_MESSAGE(WIDCOMM_WM_ON_REMOTE_DISCONNECTED_L2CAP, OnRemoteDisconnectedMessage)
...
#ifdef MTU_TESTING
        rc = Connect(m_pL2CapIf,m_BdAddr, CONNECT_REQ_MTU );
#else
...
#ifdef MTU_TESTING
    CString msg;
    msg.Format("initial Running Clock: remote %d", p_dlg->m_RemoteMtu);
    ...
    do
    {
        ...
        if (p_dlg->m_IsConnected)
        {
            ...
            if(bFirstTime)
            {
                bFirstTime = FALSE;
                // send a big buffer
                UINT16 bufsize = FIRST_BUFFER_TEST_SEND_SIZE;
                UINT8 *buffer = new UINT8[bufsize];
                p_dlg->Write((void *)(buffer), bufsize, &len_written);
                msg.Format("First Write: remote %d, buf: %d, written, %d", p_dlg->m_RemoteMtu, bufsize, len_written);
                p_dlg->SetDlgItemText(IDC_STATUS_TEXT, msg);
                delete [] buffer;
                Sleep(10000);
            }
            msg.Format("b4 Write: remote %d", p_dlg->m_RemoteMtu);
            p_dlg->SetDlgItemText(IDC_STATUS_TEXT, msg);
#endif
...
#ifdef MTU_TESTING
    CString msg;
    msg.Format("OnIncoming: remote %d", this->m_RemoteMtu);
    SetDlgItemText(IDC_STATUS_TEXT, msg);
    Sleep(500);
    
    Accept(ACCEPT_MTU);
    msg.Format("after Accept: remote %d", this->m_RemoteMtu);
    SetDlgItemText(IDC_STATUS_TEXT, msg);
    Sleep(500);
#else
    Accept();
...
void CBlueTimeDlg::OnConnected ()
{
    ...
    ::PostMessage(g_hDialog, WIDCOMM_WM_ON_CONNECTED_L2CAP, 0, 0);
}
...
LRESULT CBlueTimeDlg::OnConnectedMessage(WPARAM wParam, LPARAM lParam)
{
    BD_ADDR m_BdAddr;
#ifdef MTU_TESTING
    CString msg;
    msg.Format("OnConnected: remote %d", this->m_RemoteMtu);
    SetDlgItemText(IDC_STATUS_TEXT, msg);
    Sleep(500);
    bFirstRead = TRUE;
#endif
...
void CBlueTimeDlg::OnDataReceived(void *p_data, UINT16 length)
{
    ...
    if(bFirstRead)
    {
        bFirstRead = FALSE;
        CString Firstmsg;
        Firstmsg.Format("First msg rcv'd, length: %d", length);
        SetDlgItemText(IDC_STATUS_TEXT, Firstmsg);
        return;
    }
#endif
...
{
    ::PostMessage(g_hDialog, WIDCOMM_WM_ON_REMOTE_DISCONNECTED_L2CAP, (WPARAM)reason, 0);
}

LRESULT CBlueTimeDlg::OnRemoteDisconnectedMessage(WPARAM wParam, LPARAM lParam)
{
    // Set text on button and flag to redefine it as the
    // way to restart a session.
    //
