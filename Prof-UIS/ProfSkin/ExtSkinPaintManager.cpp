			...
			{ // if skinned color is not hollow
				...
				if( pCurrFont->GetSafeHandle() != NULL )
				{
					...
					else
					{
						...
						COLORREF clrText = GetColor( COLOR_3DSHADOW, _ppbd.m_pHelperSrc, _ppbd.m_lParam );
						COLORREF clrShadow = GetColor( COLOR_3DHILIGHT, _ppbd.m_pHelperSrc, _ppbd.m_lParam );
						CPoint ptOffset( 1, 1 );
						...
						if( nAmpIndex >= 0 )
						{
							...
							{
								pen.CreatePen( PS_SOLID, 0, dc.GetTextColor() );
							...
							else
							{
								...
								pen.CreatePen( PS_SOLID, 0, clrShadow );
								pOldPen = dc.SelectObject( &pen );
								dc.MoveTo( ptLineFrom );
								dc.LineTo( ptLineTo );
								pen.DeleteObject();
								...
								pen.CreatePen( PS_SOLID, 0, clrText );
								dc.SelectObject( &pen );
								dc.MoveTo( ptLineFrom );
								dc.LineTo( ptLineTo );
								pen.DeleteObject();
						...
						{
							rcTextLocation.OffsetRect( ptOffset.x, ptOffset.y );

							COLORREF clrTextOld = dc.SetTextColor( clrShadow );
							dc.DrawText(
								sBtn,
								sBtn.GetLength(),
								rcTextLocation,
			...
			COLORREF clrText =
				...
				CFont * pCurrFont =
					( _pgd.m_bSideBar && (!_pgd.m_bFloating) )
						? ( (!_pgd.m_bHorz) ? &m_FontNormalBC : &m_FontNormalVertXBC )
						: ( (!_pgd.m_bHorz) ? &m_FontBoldBC : &m_FontBoldVertXBC )
					;
				CFont * pOldFont =
