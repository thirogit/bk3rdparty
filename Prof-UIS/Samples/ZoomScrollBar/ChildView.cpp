	...
	if( m_nHT != nHT )
	{
		...
		if( m_nHT >= 0 )
		{
			...
			strTipText.Format(
				...
				_object.m_strURL.IsEmpty() ? _T("") : _T("\n"),
...
CString strPaneTextNew;
	if( strName != NULL )
		strPaneTextNew.Format(
			_T("  Name: %s   Longitude: %lf   Latitude: %lf"),
			strName,
			lfLongitude,
			lfLatitude
			);
...
INT nPaneIndex = pMainFrame->m_wndStatusBar.CommandToIndex( ID_ZOOM_VALUE_PANE );
	if( nPaneIndex > 0 )
	{
		...
		strZoomPaneValue.Format( _T(" %3d %%"), nZoomPaneValue );
			...
			{ // if show at least any text labels
				...
				for( nObjectIndex = 0; nObjectIndex < nObjectCount; nObjectIndex ++ )
				{
					...
					else
					{
						...
						dc.DrawText( LPCTSTR(_object.m_strName), _object.m_strName.GetLength(), rcText, DT_SINGLELINE|DT_LEFT|DT_TOP|DT_NOCLIP );
