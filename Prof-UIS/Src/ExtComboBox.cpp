...
{
	ASSERT_VALID( this );
//	return (new CExtComboBoxPopupListBox);
...
LRESULT lResult = DefWindowProc( CB_ADDSTRING, wParam, lParam );
	if( lResult != CB_ERR )
		LbItemInsert( __EXT_MFC_SAFE_LPCTSTR ( lParam ), (INT)lResult );
	return lResult;
}
...
LB_ITEM * pLbItem = LbItemGet( nItem );
	...
	else
	{
		...
		if( ! sText.IsEmpty() )
		{
			...
			ASSERT( hFont != NULL );
			HFONT hOldFont = (HFONT)::SelectObject( dc.m_hDC, hFont );
			dc.DrawText( sText, &rcText, nFormat );
			::SelectObject( dc.m_hDC, hOldFont );
...
CRect rc;
	...
	CExtComboBox::_OnDrawComboImpl( bPressed, bHover, pDC );
	if( bCallReleaseDC )
		ReleaseDC( pDC );
}
...
HFONT hOldFont = (HFONT)::SelectObject( pDIS->hDC, hFont );
	...
	::DrawText( pDIS->hDC, _T(" ") + strText, strText.GetLength() + 1, &rcText, DT_SINGLELINE|DT_VCENTER|DT_END_ELLIPSIS );
	if( bFR )
