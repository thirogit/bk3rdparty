...
HBRUSH CExtEditBase::CtlColor( CDC* pDC, UINT nCtlColor )
{
		...
		)
	{
		...
		{
			COLORREF clrSysText = PmBridge_GetPM()->GetColor( bDisabled ? CExtPaintManager::CLR_TEXT_DISABLED : COLOR_WINDOWTEXT, this );
			clrText = clrSysText;
		}
		COLORREF clrBk = GetBkColor();
		if( clrBk == COLORREF(-1L) )
		{
			COLORREF clrSysBk = PmBridge_GetPM()->GetColor( ( bReadOnly || bDisabled ) ? CExtPaintManager::CLR_3DFACE_OUT : COLOR_WINDOW, this );
			clrBk = clrSysBk;
		}
