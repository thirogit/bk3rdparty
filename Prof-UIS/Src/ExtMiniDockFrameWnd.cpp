...
void CExtMiniDockFrameWnd::OnDisplayChange(UINT nImageDepth, int cxScreen, int cyScreen)
{
	CMiniDockFrameWnd::OnDisplayChange( nImageDepth,  cxScreen, cyScreen);
//CExtPaintManager * pPM = PmBridge_GetPM();
...
CExtControlBar * pExtBar = GetControlBarExt();
	if( pExtBar != NULL )
	{
		CExtPopupMenuTipWnd * pATTW =
			pExtBar->OnAdvancedPopupMenuTipWndGet();
		if( pATTW == NULL )
...
{
	if( GetSafeHwnd() == NULL || m_bInRecalcLayout )
		return;
//CExtControlBar * pExtBar = GetControlBarExt();
//	if(		pExtBar != NULL
//		&&	(! pExtBar->IsFixedMode() )
//		)
//	{
//		if( (GetStyle() & __REMOVED_NONFIXMODE_STYLES) != 0 )
//			ModifyStyle( __REMOVED_NONFIXMODE_STYLES, 0 );
//	}
...
HINSTANCE hInst = ::AfxGetInstanceHandle();
	if( ! ::GetClassInfo(
			hInst,
			__EXT_MINIDOCKFRAMEWND_CLASS_NAME,
			&_wndClassInfo
			)
		)
	{
