	...
	)
{
	...
	{
		if( g_PaintManager.m_nInsallPaintManagerCounter != 0 )
		{
...
WINDOWPLACEMENT _wp;
	...
	CExtNcFrameImpl::stat_GetWindowPlacement(
		hWndOwn,
		_wp
		);
...
CWnd * pWndPermanent = CWnd::FromHandlePermanent( hWndOwn );
	...
	if( m_pNcFrameImpl->NcFrameImpl_IsDwmCaptionReplacement() )
		...
		CExtNcFrameImpl::stat_SetWindowPlacement(
			hWndOwn,
			_wp
			);
...
bool bContinueRecalc = true;
	...
	if( ! m_pNcFrameImpl->NcFrameImpl_IsDwmBased() )
	{
			...
			)
		{
			...
			::CopyRect( &_wp2.rcNormalPosition, &rcMax );
...
// 		if( ! _mp.m_bPrimaryMonitor )
// 		{
// 			CPoint ptPrimary( 100, 100 );
// 			CExtPaintManager::monitor_parms_t _mpPrimary;
// 			CExtPaintManager::stat_GetMonitorParms( _mpPrimary, ptPrimary );
// 			if( ! _mpPrimary.m_bPrimaryMonitor )
// 			{
...
// 			else
// 			{
// 				pMMI->ptMaxPosition.x = _mpPrimary.m_rcWorkArea.left + rcBorders.left;
// 				pMMI->ptMaxPosition.y = _mpPrimary.m_rcWorkArea.top + rcBorders.top;
// 			}
// 		}
