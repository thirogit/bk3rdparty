...
bool bFadeOut = _IsFadeOutAnimation();
		...
		)
	{
		...
		HBITMAP hBmpScreenSrcAlt =
			CExtPaintManager::stat_GetScreenSurfacePart(
				rcCapture
				);
...
CPalette * pOldPalette = NULL, * pOldPalette2 = NULL;
    ...
    {
        pOldPalette =
			dcPaint.SelectPalette( &PmBridge_GetPM()->m_PaletteWide, FALSE );
        dcPaint.RealizePalette();
    }
	PmBridge_GetPM()->AdjustMenuDC(
		this,
		dcPaint,
		false
		);
...
CExtMemoryDC mdc;
	if( bUseBackBuffer )
	{
		...
		{
			pOldPalette2 =
				mdc.SelectPalette( &PmBridge_GetPM()->m_PaletteWide, FALSE );
			mdc.RealizePalette();
		}
		PmBridge_GetPM()->AdjustMenuDC(
			this,
...
DRAWBACKGROUNDDATA _dbkgnd(
	...
	if( ! _dbkgnd.m_bCustomBorderDrawn )
		PmBridge_GetPM()->PaintMenuBorder(
			dc,
			rcClient,
			this
			);
	...
	if( (TrackFlagsGet()&TPMX_RIBBON_RESIZING) != 0 )
	{
		...
		rcResizingArea.top = rcResizingArea.bottom - nMarginHeight;
		PmBridge_GetPM()->MenuBottomResizingMarginPaint(
			dc,
			rcResizingArea,
			this
			);
...
CPoint point( 0, 0 );
	...
	if( m_bExpandAvailable )
	{
		...
		bool bPushed = bHover;
		PmBridge_GetPM()->PaintMenuExpandButton(
			dc,
			rcExpand,
			bHover,
			bPushed,
	...
	if( bTearOff )
	{
		...
		if( bHover && _FindCustomizeMode() )
			bHover = false;
		bool bPushed = bHover;
		PmBridge_GetPM()->PaintMenuTearOffButton(
			dc,
			rcTearOff,
			bHover,
			bPushed,
	...
	if( m_bScrollingAvailable )
	{
		if( m_nDyScrollOffset != 0 )
		{
			...
			PmBridge_GetPM()->PaintMenuScrollButton(
				dc,
				bHover,
				bPushed,
				true,
		...
		if( vi.m_bHelperIsSeparator )
		{
			...
			if( bPrevToolButton )
			{
				...
				PmBridge_GetPM()->PaintMenuSeparator(
					...
					(		mi.IsForceNoLeftGradient()
						||	IsNoHighlightIconArea()
						) ? true : false,
					this
					);
...
CExtMemoryDC mdc;
	if( bUseBackBuffer )
	{
		mdc.__InitMemoryDC(
			&dcPaint,
			&rcClient, // &rcRealClient, // &rcClient,
///			CExtMemoryDC::MDCOPT_RTL_COMPATIBILITY|
			CExtMemoryDC::MDCOPT_TO_MEMORY
...
static bool g_bWndClassRegistered = false;
	if( ! g_bWndClassRegistered )
	{
		...
		{
			_wndClassInfo.style = CS_OWNDC|CS_GLOBALCLASS|CS_HREDRAW|CS_VREDRAW;
