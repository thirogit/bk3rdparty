...
void CExtResizableDialog::PreSubclassWindow() 
{
	...
	HookSpyRegister( __EHSEF_MOUSE_ALL_WITHOUT_WHEEL|__EHSEF_WND_PROC_IN|__EHSEF_WND_PROC_IN|__EHSEF_PRE_TRANSLATION );
...
//			CTabCtrl * pTabCtrl = CExtResPS::GetTabControl();
			...
			for( ; pWnd != NULL; pWnd = pWnd->GetWindow(GW_HWNDNEXT)  )
			{
				...
				if( sClassName == LPCTSTR( _T("button") ) )
				{
					...
					if( this->FromHandlePermanent( pWnd->GetSafeHwnd() ) == NULL )
					{
