	...
	if( (dwStyle&UDS_AUTOBUDDY) != 0 )
	{
		...
		if( pWnd != NULL )
		{
			...
			if( !lstrcmpi( szCompare, szEdit ) )
			{
				...
				bBuddyToEdit = (bSpinAlignRight || bSpinAlignLeft) ? true : false;
			}
		}
	}
	return bBuddyToEdit;
		...
		)
	{
		...
		if( ! bAnimationLocked )
		{
			AnimationClient_CacheGeneratorLock();
 			AnimationClient_CacheNextStateMinInfo(
				false,
				( bButtonUpHoveredNext || bButtonDownHoveredNext )
					? __EAPT_BY_HOVERED_STATE_TURNED_ON : __EAPT_BY_HOVERED_STATE_TURNED_OFF
				);
		}
