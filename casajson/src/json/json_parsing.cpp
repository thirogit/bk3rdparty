...
public:
    ...
    json::value ParseValue(typename JSON_Parser<CharType>::Token &first)
    {
#ifndef _WIN32
...
namespace
{
    ...
    {
        return swprintf(ptr, n, L"%llu", val64);
    }
    static int __attribute__((__unused__)) print_llu(wchar_t* ptr, size_t n, unsigned long val64)
    {
        return swprintf(ptr, n, L"%lu", val64);
    }
...
static json::value _parse_stream(utility::istream_t &stream)
{
    json::details::JSON_StreamParser<utility::char_t> parser(stream);
    json::details::JSON_Parser<utility::char_t>::Token tkn;

    parser.GetNextToken(tkn);
    if (tkn.m_error)
    {
        json::details::CreateException(tkn, utility::conversions::to_string_t(tkn.m_error.message()));
    }
    ...
    else if (tkn.kind != json::details::JSON_Parser<utility::char_t>::Token::TKN_EOF)
    {
        json::details::CreateException(tkn, _XPLATSTR("Left-over characters in stream after parsing a JSON value"));
    }
    return value;
}

static json::value _parse_stream(utility::istream_t &stream, std::error_code& error)
{
    json::details::JSON_StreamParser<utility::char_t> parser(stream);
    json::details::JSON_Parser<utility::char_t>::Token tkn;
    ...
    if (tkn.kind != json::details::JSON_Parser<utility::char_t>::Token::TKN_EOF)
    {
...
#ifdef _WIN32
static json::value _parse_narrow_stream(std::istream &stream)
{
...
static json::value _parse_narrow_stream(std::istream &stream, std::error_code& error)
{
    json::details::JSON_StreamParser<char> parser(stream);
    json::details::JSON_StreamParser<char>::Token tkn;
...
json::value json::value::parse(const utility::string_t& str)
{
    json::details::JSON_StringParser<utility::char_t> parser(str);
    json::details::JSON_Parser<utility::char_t>::Token tkn;
...
json::value json::value::parse(const utility::string_t& str, std::error_code& error)
{
    json::details::JSON_StringParser<utility::char_t> parser(str);
    json::details::JSON_Parser<utility::char_t>::Token tkn;
...
json::value json::value::parse(utility::istream_t &stream)
{
    return _parse_stream(stream);
}

json::value json::value::parse(utility::istream_t &stream, std::error_code& error)
{
    return _parse_stream(stream, error);
}

#ifdef _WIN32
json::value json::value::parse(std::istream& stream)
{
    return _parse_narrow_stream(stream);
}

json::value json::value::parse(std::istream& stream, std::error_code& error)
{
