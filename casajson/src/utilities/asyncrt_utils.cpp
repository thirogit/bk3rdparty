...
{
#ifndef _WIN32
bool parse_time(const utility::string_t &s,const utility::string_t &fmt, struct tm* tm)
{
	std::wstringstream ss(s);
	pt::wtime_input_facet *df = new pt::wtime_input_facet(fmt);
...
utility::string_t datetime::to_string(date_format format) const
{
    ...
    wchar_t output[max_dt_length+1] = {0};
    ...
    if (format != RFC_1123 && frac_sec > 0)
    {
        ...
        // This way, '1200' becomes '00012'
        wchar_t buf[9] = { 0 };
        swprintf(buf, sizeof(buf), L".%07ld", (long int)frac_sec);
        // trim trailing zeros
        for (int i = 7; buf[i] == '0'; i--) buf[i] = '\0';
        // format the datetime into a separate buffer
        wchar_t datetime_str[max_dt_length+1] = {0};
        wcsftime(datetime_str, sizeof(datetime_str), L"%Y-%m-%dT%H:%M:%S", &datetime);
        // now print this buffer into the output buffer
        swprintf(output, sizeof(output), L"%s%sZ", datetime_str, buf);
    }
    else
    {
        wcsftime(output, sizeof(output),
            format == RFC_1123 ? L"%a, %d %b %Y %H:%M:%S GMT" : L"%Y-%m-%dT%H:%M:%SZ",
            &datetime);
    }
...
void extract_fractional_second(const utility::string_t& dateString, utility::string_t& resultString, uint64_t& ufrac_second)
{
    ...
    if (resultString.size() > 2 && resultString[resultString.size() - 1] == _U('Z'))
    {
...
datetime __cdecl datetime::from_string(const utility::string_t& dateString, date_format format)
{
    ...
    if (format == RFC_1123)
    {
     	details::parse_time(input.data(), L"%a, %d %b %Y %H:%M:%S GMT", &output);
    }
    else
    {
        // Try to extract the fractional second from the timestamp
	...
	bool result = details::parse_time(input.data(), L"%Y-%m-%dT%H:%M:%SZ", &output);

        if (!result)
        {
            result = details::parse_time(input.data(), L"%Y%m%dT%H:%M:%SZ", &output);
        }
        if (!result)
        {
            ...
            output.tm_mon = 1;
            output.tm_mday = 1;
            result = details::parse_time(input.data(), L"%H:%M:%SZ", &output);
        }
        if (!result)
        {
            result = details::parse_time(input.data(), L"%Y-%m-%d", &output);
        }
        if (!result)
        {
            result = details::parse_time(input.data(), L"%Y%m%d", &output);
