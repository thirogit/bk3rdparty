...
{ 
  ...
  sprintf(tmp, "&#%lu;", c);
	...
	char* outbuf = tmp;
	size_t outbufleft = buff_size;

	const char* inbuf2 = inbuf;
	iconv(soap->convToUtf8,&inbuf2,&inbytesleft,&outbuf,&outbufleft);
	tmp[buff_size-outbufleft] = 0;
#endif
...
{ 
  ...
  const char *inbuf2 = inbuf;
  char *outbuf2 = outbuf;
  iconv(soap->convFromUtf8,&inbuf2,&inbufleft,&outbuf2,&outbufleft);
  return outbuf[0];
...
#ifdef BK_POWERED
	iconv_close(soap->convFromUtf8);
	iconv_close(soap->convToUtf8);
#endif
...
#ifdef BK_POWERED
	soap->convFromUtf8 = iconv_open(locale_charset(),"UTF-8");
	soap->convToUtf8 = iconv_open("UTF-8",locale_charset());
#endif
}
#endif
